        ...
        <div className="bg-gray-800  backdrop-filter backdrop-blur-sm bg-opacity-30 rounded rounded-xl border border-[#6cbfe1] items-center flex flex-col w-[570px] pt-12 mx-auto font-['Poppins']">
            ...
            <div className="flex flex-col bg-[#010f20] px-12 pb-5 pt-14 rounded-xl bg-opacity-60 border border-gray-600">
                ...
                <button className="" onClick={togglePlayPause}>
                    ...
                    {isPlaying ? 
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mx-auto my-4 text-[#6cbfe1]" viewBox="0 0 20 20" fill="currentColor">
                ...
                <input className='mb-3 form-range appearance-none w-full h-3 p-0 bg-sky-700 bg-opacity-30 focus:outline-none focus:ring-0 focus:shadow-none rounded-lg' type="range" defaultValue='0' ref={progressBar} onChange={changeRange}/>
                <div className='flex justify-between'>
                  <div className='text-gray-400 font-semibold'>{calculateTime(currentTime)}</div>
                  <div className="text-gray-400 font-semibold">{(duration && !isNaN(duration)) && calculateTime(duration)}</div>
                </div>
